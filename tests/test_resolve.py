...
def test_resolution_merges_duplicates(initialized_corpus):
    ...
    conn.execute(
        ...
        ("test.md", "abc", 10, 0.0, now),
    )
    conn.execute(
        "INSERT INTO chunks (document_id, chunk_index, content, char_start, char_end) VALUES (1, 0, 'test', 0, 4)",
    )
    conn.execute(
    ...
    conn.execute(
        ...
        ("Brian Anderson", "Person", now, now),
    )
    conn.execute(
        "INSERT INTO entities (canonical_name, entity_type, attributes, created_at, updated_at) VALUES (?, ?, '{}', ?, ?)",
        ("B. Anderson", "Person", now, now),
    )
    ...
    conn.execute(
        "INSERT INTO entity_mentions (entity_id, surface_form, chunk_id, extraction_run_id, confidence) VALUES (1, 'Brian Anderson', 1, 1, 0.9)",
    )
    conn.execute(
        "INSERT INTO entity_mentions (entity_id, surface_form, chunk_id, extraction_run_id, confidence) VALUES (2, 'B. Anderson', 1, 1, 0.9)",
    )
    conn.execute(
        "INSERT INTO vec_entity_mentions (mention_id, embedding) VALUES (1, ?)", (vec_bytes1,)
    )
    conn.execute(
        "INSERT INTO vec_entity_mentions (mention_id, embedding) VALUES (2, ?)", (vec_bytes2,)
    )
    conn.commit()
